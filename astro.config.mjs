...
export default defineConfig({
  site: SITE_URL,
  output: 'static',
  /* URLs have no trailing slash — the shape Webflow used, that Google indexed
     and that every canonical declares. 'file' output (about-us.html) is what
     makes Netlify serve /about-us directly; with the default 'directory'
     output it 301'd /about-us → /about-us/, so every canonical pointed at a
     redirect ("Alternate page with proper canonical tag" in Search Console). */
  trailingSlash: 'never',
  build: { format: 'file' },
  integrations: [
    react(),
    sitemap({
      ...
      // Thank-you pages are post-submit confirmations (noindex, like session).
      filter: (page) => !/\/(session|thank-you)\/?$/.test(new URL(page).pathname),
